...
export default function OfficerDashboard({ incidents, officerInfo }: OfficerDashboardProps) {
  const [selectedPeriod, setSelectedPeriod] = useState('today');

  // Calculate statistics
  const totalReports = incidents.length;
  const pendingReports = incidents.filter(i => i.status === 'pending').length;
  const underReviewReports = incidents.filter(i => i.status === 'under_review').length;
  const investigatingReports = incidents.filter(i => i.status === 'investigating').length;
  const resolvedReports = incidents.filter(i => i.status === 'resolved').length;
  const highPriorityReports = incidents.filter(i => i.severity === 'high' || i.severity === 'critical').length;
  ...
  const recentActivity = [
    { action: 'New report submitted', location: 'Kampala Central', time: '5 min ago', type: 'new' },
    { action: 'Report status updated', location: 'Nakawa Division', time: '15 min ago', type: 'update' },
    { action: 'Investigation completed', location: 'Kawempe Division', time: '1 hour ago', type: 'completed' },
    { action: 'High priority alert', location: 'Makindye Division', time: '2 hours ago', type: 'alert' }
  ];
          ...
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600">
                Welcome back, Officer {officerInfo.name} • Badge: {officerInfo.badgeNumber}
              </p>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-sm border">
              <div className="p-6 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-600">Latest updates from your jurisdiction</p>
              </div>
              <div className="p-6">
                ...
                <div className="space-y-4">
                  {districtStats.map((district, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{district.district}</p>
                        <p className="text-xs text-gray-600">{district.reports} reports</p>
                      </div>
                      <div className="flex items-center">
                        <span className={`w-2 h-2 rounded-full mr-2 ${
                          district.trend === 'up' ? 'bg-red-500' :
                          district.trend === 'down' ? 'bg-green-500' :
                          'bg-gray-400'
                        }`}></span>
                        <svg className={`w-4 h-4 ${
                          district.trend === 'up' ? 'text-red-500 rotate-180' :
                          district.trend === 'down' ? 'text-green-500' :
                          'text-gray-400'
            ...
            <div className="bg-white rounded-xl shadow-sm border mt-6">
              ...
              <div className="p-6 space-y-3">
                ...
                <button className="w-full text-left p-3 rounded-lg hover:bg-gray-50 transition-colors flex items-center">
                  ...
                  <span className="text-sm font-medium">Open Complaint Chats</span>
                </button>
                <button className="w-full text-left p-3 rounded-lg hover:bg-gray-50 transition-colors flex items-center">
